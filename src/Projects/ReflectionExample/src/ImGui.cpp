...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        boost::mp11::mp_for_each<boost::describe::describe_members<MyActor, boost::describe::mod_any_access>>([&]<typename T0>(T0 member)
        {
            // member_descriptor.pointer is a pointer-to-member
            using member_type = std::decay_t<decltype(Actor.*member.pointer)>;
            constexpr const char* MemberName = T0::name;
            auto& MemberValue = Actor.*member.pointer;
            ShowImGuiWidget<member_type>(MemberName, MemberValue);
        });
        ImGui::End();
