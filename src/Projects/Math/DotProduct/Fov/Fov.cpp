...
void MakeFOVCalculationWithDotProduct(const sf::CircleShape& player, sf::Vector2f heroDirection, const sf::Vector2f& enemyPos, sf::Vector2f& playerToEnemy, float& dot,
                                      std::string& inRangeMessage, bool& isInFOV, const float fovDegrees = 90.0f, const float fovDistance = 150.0f)
{
...
void DrawFOVVisualization(sf::RenderWindow& window, const sf::Vector2f& playerPos, const sf::Vector2f& direction, const float fovDegrees, const bool enemyInFOV,
...
MakeFOVCalculationWithDotProduct(hero, HeroDirection, enemy.getPosition(), playerToEnemyRelative, dot, FovStatusMessage, isInFOV, fovDegrees, fovDistance);
